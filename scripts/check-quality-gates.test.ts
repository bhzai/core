...
describe("checkNarration", () => {
	it("detects TASK_XXXX narration", () => {
		...
		const errors = checkNarration("src/v2/foo.ts", content)
...
describe("checkFileLength", () => {
	it("fails files exceeding the line limit", () => {
		...
		const errors = checkFileLength("src/v2/long.ts", content, 500)
		expect(errors.length).toBe(1)
	...
	it("passes files within the line limit", () => {
		...
		const errors = checkFileLength("src/v2/short.ts", content, 500)
		expect(errors.length).toBe(0)
	})
})
...
describe("checkTestImports", () => {
	it("fails tests importing _-prefixed symbols", () => {
		...
		const sf = ts.createSourceFile("src/v2/foo.test.ts", code, ts.ScriptTarget.ES2022, true)
		const errors = checkTestImports("src/v2/foo.test.ts", sf)
		expect(errors.length).toBe(1)
...
describe("checkSourceFile", () => {
	it("fails exported symbols missing JSDoc", () => {
		...
		const sf = ts.createSourceFile("src/v2/foo.ts", code, ts.ScriptTarget.ES2022, true)
		const errors = checkSourceFile("src/v2/foo.ts", sf, false)
		expect(errors.length).toBe(1)
